...
public class StockWatcher implements EntryPoint {
  ...
  private void addStock() {
    ...
    if (persistStock(symbol)) {
      ...
      /*
       * 
       * // Add a button to remove this stock from the table. Button
       * removeStockButton = new Button("x");
       * removeStockButton.addStyleDependentName("remove");
       * removeStockButton.addClickHandler(new ClickHandler() { public void
       * onClick(ClickEvent event) { purgeStock(event,symbol); // int
       * removedIndex = stocks.indexOf(symbol); // stocks.remove(removedIndex);
    ...
    stockPriceSvc.get(primaryKey, new AsyncCallback<StockPrice>() {
      ...
      public void onSuccess(StockPrice stock) {
        if (stock != null) {
          ...
          stockPriceSvc.delete(stock, callback);// (new StockPrice(symbol, 1),
                                                // callback);//(stocks.toArray(new
                                                // String[0]), callback);
  ...
  /*
   * @ return true if stock exists retunr false if stock doesn't exist
   */
  private boolean checkExistingStock(String symbol) {
    ...
    // Make the call to the stock price service.
    stockPriceSvc.get(symbol, callback);// (stocks.toArray(new String[0]),
                                        // callback);
  ...
  private boolean persistStock(final String symbol) {
    ...
    // Make the call to the stock price service.
    stockPriceSvc.saveOrUpdate(new StockPrice(symbol), callback);// (stocks.toArray(new
                                                                 // String[0]),
                                                                 // callback);
    return bVal;
  }
  ...
  private void loadStocks() {
    ...
    // Make the call to the stock price service.
    stockPriceSvc.getAllStocks(callback);// (stocks.toArray(new String[0]),
                                         // callback);
    // refreshWatchList();
  }
  ...
  private void updateTable(final StockPrice price) {
    ...
    Label changeWidget = new Label("");// (Label)stocksFlexTable.getWidget(row,
                                       // 2);
